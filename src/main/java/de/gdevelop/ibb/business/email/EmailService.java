...
import static de.gdevelop.ibb.business.email.EmailTemplate.EmailTemplatePart.BODY_TITLE;
import static de.gdevelop.ibb.business.email.EmailTemplate.EmailTemplatePart.SUBJECT_CONTENT;
...
import de.gdevelop.ibb.business.email.EmailTemplate.EmailTemplatePart;
import de.gdevelop.ibb.business.exception.SystemException;
...
public abstract class EmailService  {
	...
	private static final List<String> ALLOWED_FROM_DOMAINS = Arrays.asList("ibb-d.de");
	...
	@PostConstruct
	public void init() {
		defaultEmailUser = new EmailUser(fromEmail, "IBB Team");
	}
	...
	public void sendTemplate(EmailTemplate templateEmail, Map<String, Object> messageParameters) {
		...
		if (templateEmail.getFromUser() == null) {
			...
			if (email != null && email.contains("@") && !ALLOWED_FROM_DOMAINS.contains(email.substring(email.indexOf("@") + 1))) {
				templateEmail.setFromUser(new EmailUser(defaultEmailUser.getEmail(), fromUser.getFullName() + " via IBB Verwaltungssoftware"));
